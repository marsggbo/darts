...
parser.add_argument('--data_type', type=int, default=0,
                    help='''data type. 0: image folder; 1: pkl files; 
					You can define the specific method to load the data in data_utils.py''')
parser.add_argument('--output_classes', type=int,
                    default=10, help='output classes')
...
log_format = '%(asctime)s - %(name)s - %(levelname)s - %(lineno)d - %(module)s - %(message)s'
...
def main():
    ...
    model = Network(args.init_channels, OUTPUT_CLASSES, args.layers, criterion)
    model = model.cuda()
    ...
    train_queue, valid_queue, test_queue = get_dataloaders(args)
